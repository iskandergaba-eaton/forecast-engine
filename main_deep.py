...
if __name__ == "__main__":
    ...
    influence = 1
    # Data loading
    DCs = ["simtcsvc01", "usltccnvcenter4", "usltccnvcenter5", "usltcsvcenter2", "usltcsvcenter3"]
    versions_gap = ["19-02-2022-gaps"]
    ...
    for dc in DCs :
        powers_mean_noisy[dc] = add_noise(dfs_mean_gap[dc], colname ='power')
        print('noise added on ', dc)
    ...
    alg = EatonPowerForecastModel(time_window, autoencoder_activation, model_activation)
    Xtrain, ytrain = alg.initialize_dataset(y_train)
    Xval, yval = alg.initialize_dataset(y_val)

    # Scale down the data
    xtrain_std, ytrain_std, sc_xtrain, sc_ytrain = alg.prepare_data_autoencoder(Xtrain, ytrain)
    xval_std, yval_std, sc_xval, sc_yval = alg.prepare_data_autoencoder(Xval, yval)

    X_train, Y_train = alg.create_variable_for_model(xtrain_std, time_window), ytrain_std[time_window:]
    X_val, Y_val =  alg.create_variable_for_model(xval_std, time_window), yval_std[time_window:]
    ...
    plt.plot(train_id, y_val[train_id], alpha =0.5,color = 'green',label ='Observed past')
    plt.plot(val_id, y_val[val_id],alpha =0.5,color ='red', label ='observed future')
